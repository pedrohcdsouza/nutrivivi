...
function BotanicalIllustration() {
  return (
    <svg viewBox="0 0 280 420" fill="none" className="botanical-svg" aria-hidden="true">
      <path d="M140 410 C140 410 138 310 142 210 C146 130 140 70 140 48"
        stroke="rgba(168,213,186,0.35)" strokeWidth="1.5" />
      <path d="M140 230 C98 210 58 178 48 136 C38 92 76 80 108 100 C128 114 138 190 140 230Z"
        fill="rgba(168,213,186,0.12)" stroke="rgba(168,213,186,0.30)" strokeWidth="1.5" />
      <path d="M140 230 C118 198 80 158 56 136"
        stroke="rgba(168,213,186,0.20)" strokeWidth="1" />
      <path d="M140 290 C188 262 234 224 242 178 C250 130 210 118 180 140 C160 158 143 248 140 290Z"
        fill="rgba(168,213,186,0.10)" stroke="rgba(168,213,186,0.25)" strokeWidth="1.5" />
      <path d="M140 290 C168 256 214 218 242 178"
        stroke="rgba(168,213,186,0.16)" strokeWidth="1" />
      <path d="M140 168 C112 150 84 122 82 96 C80 72 106 66 126 82 C137 94 140 148 140 168Z"
        fill="rgba(168,213,186,0.08)" stroke="rgba(168,213,186,0.22)" strokeWidth="1" />
      <path d="M140 138 C164 118 192 94 196 68 C200 44 172 38 155 56 C145 70 140 120 140 138Z"
        fill="rgba(168,213,186,0.08)" stroke="rgba(168,213,186,0.22)" strokeWidth="1" />
      <path d="M140 348 C106 326 70 298 68 264 C66 228 98 222 124 242 C136 255 140 320 140 348Z"
        fill="rgba(168,213,186,0.07)" stroke="rgba(168,213,186,0.18)" strokeWidth="1" />
        ...
        <div className="left-content">
          <div className="brand">
            <svg width="28" height="28" viewBox="0 0 32 32" fill="none" aria-hidden="true">
              <path d="M16 30 C8 24 4 16 6 8 C8 2 16 0 22 4 C28 8 30 18 24 24 C21 27 18 29 16 30Z"
                fill="rgba(168,213,186,0.55)" />
              <path d="M16 30 C14 22 12 14 14 8"
          ...
          <div className="success-state">
            <div className="success-icon">
              <svg width="64" height="64" viewBox="0 0 64 64" fill="none">
                ...
                <path d="M18 32 L28 42 L46 22" stroke="#3a7d5a" strokeWidth="3"
                  strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <h2 className="success-title">Anamnese enviada!</h2>
            <p className="success-desc">
              Sua nutricionista já recebeu seus dados e irá analisá-los em breve.
              Aguarde o contato para iniciar sua jornada de saúde.
            </p>
            <button
          ...
          <>
            <div className="form-header">
              ...
              <p className="form-subtitle">
                Suas respostas são confidenciais e essenciais para um atendimento
            ...
            >
              ...
              <SectionCard number={1} title="Identificação do Paciente" className="anim-1">
                ...
                <div className="form-row">
                  <Form.Item name="birth_date" label="Data de Nascimento"
                    rules={[{ required: true, message: "Obrigatório" }]} style={{ flex: 1 }}>
                ...
                <Form.Item name="consultation_reason" label="Motivo da Consulta"
                  rules={[{ required: true, message: "Obrigatório" }]}>
              ...
              <SectionCard number={2} title="Antropometria Inicial" className="anim-2">
                <div className="form-row">
                  <Form.Item name="weight_kg" label="Peso Atual (kg)"
                    rules={[{ required: true, message: "Obrigatório" }]} style={{ flex: 1 }}>
                    <InputNumber size="large" min={1} max={500} step={0.1}
                      style={{ width: "100%" }} placeholder="Ex: 70.5" />
                  </Form.Item>
                  <Form.Item name="height_cm" label="Altura (cm)"
                    rules={[{ required: true, message: "Obrigatório" }]} style={{ flex: 1 }}>
                    <InputNumber size="large" min={30} max={300}
                      style={{ width: "100%" }} placeholder="Ex: 170" />
                  </Form.Item>
                </div>
              </SectionCard>
              ...
              <SectionCard number={3} title="Saúde e Estilo de Vida" className="anim-3">
                <div className="form-row-three">
                  <Form.Item name="sleep_quality" label="Qualidade do Sono"
                    rules={[{ required: true }]}>
                    <Select size="large">
                  ...
                  <Form.Item name="anxiety_level" label="Nível de Ansiedade"
                    rules={[{ required: true }]}>
                    <Select size="large">
                  ...
                  <Form.Item name="stress_level" label="Nível de Estresse"
                    rules={[{ required: true }]}>
                    <Select size="large">
                      <Option value={1}>1 — Muito baixo</Option>
                      <Option value={2}>2 — Baixo</Option>
                ...
                <Form.Item name="does_physical_activity" label="Pratica Atividade Física?"
                  ...
                  {({ getFieldValue }) =>
                    getFieldValue("does_physical_activity") ? (
                      <div className="conditional-block">
                        <div className="form-row">
                          <Form.Item name="activity_modality" label="Modalidade"
                            rules={[{ required: true, message: "Obrigatório" }]} style={{ flex: 1 }}>
                            <Input placeholder="Ex: Musculação, Corrida" />
                          </Form.Item>
                          <Form.Item name="activity_weekly_frequency" label="Frequência Semanal"
                            rules={[{ required: true, message: "Obrigatório" }]} style={{ flex: 1 }}>
                            <InputNumber min={1} max={14} style={{ width: "100%" }} placeholder="Vezes/semana" />
                          </Form.Item>
                        </div>
                        <div className="form-row">
                          <Form.Item name="activity_workout_time" label="Horário do Treino"
                            rules={[{ required: true, message: "Obrigatório" }]} style={{ flex: 1 }}>
                            <Input placeholder="Ex: 18:30" />
                          </Form.Item>
                          <Form.Item name="activity_duration_minutes" label="Duração (minutos)"
                            rules={[{ required: true, message: "Obrigatório" }]} style={{ flex: 1 }}>
              ...
              <SectionCard number={4} title="Suplementação Atual" className="anim-4">
                <Form.Item name="uses_supplement" label="Utiliza Algum Suplemento?"
                  valuePropName="checked">
                  <Switch />
                </Form.Item>
                <Form.Item noStyle
                  shouldUpdate={(prev, curr) => prev.uses_supplement !== curr.uses_supplement}>
                  {({ getFieldValue }) =>
                    getFieldValue("uses_supplement") ? (
                      <Form.Item name="supplements" label="Quais suplementos?"
                        rules={[{ required: true, message: "Obrigatório" }]}>
              ...
              <SectionCard number={6} title="Observações Adicionais" className="anim-6">
                <Form.Item name="additional_observations"

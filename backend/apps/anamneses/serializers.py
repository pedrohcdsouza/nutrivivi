...
class AnamneseCreateSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            "anxiety_level",
            "stress_level",
            "does_physical_activity",
            "activity_modality",
            "activity_weekly_frequency",
            "activity_workout_time",
        ...
        if data.get("does_physical_activity"):
            if not data.get("activity_modality"):
                raise serializers.ValidationError(
                    {"activity_modality": "Obrigatório se pratica atividade física."}
                )
            if not data.get("activity_weekly_frequency"):
                raise serializers.ValidationError(
                    {
                        "activity_weekly_frequency": "Obrigatório se pratica atividade física."
                    }
                )
            if not data.get("activity_workout_time"):
                raise serializers.ValidationError(
                    {
                        "activity_workout_time": "Obrigatório se pratica atividade física."
                    }
                )
            if not data.get("activity_duration_minutes"):
                raise serializers.ValidationError(
                    {
                        "activity_duration_minutes": "Obrigatório se pratica atividade física."
                    }
                )

        if data.get("uses_supplement") and not data.get("supplements"):
            raise serializers.ValidationError(
                {"supplements": "Obrigatório se usa suplemento."}
            )
